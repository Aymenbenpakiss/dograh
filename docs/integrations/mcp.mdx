...
- A Asisso API key. Generate one at [`/api-keys`](https://app.asisso.com/api-keys) (or `http://localhost:3010/api-keys` for self-hosted). See [API Keys](/configurations/api-keys).
- Your Asisso MCP endpoint:
  - Hosted: `https://app.asisso.com/api/v1/mcp/`
  - Self-hosted: `<YOUR_BACKEND_URL>/api/v1/mcp/`

The endpoint is also shown in **Platform Settings → MCP Server** inside the Asisso UI.

<Note>
If you deployed Asisso to a remote server using [`setup_remote.sh`](/deployment/docker#option-2%3A-remote-server-deployment), your endpoint is served with a self-signed SSL certificate. Claude Code will refuse MCP connections to it unless you start the CLI with TLS verification disabled:
...
- "How do I deploy Asisso on a custom domain with HTTPS?"
