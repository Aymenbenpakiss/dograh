---
title: "Update"
description: "Update your self-hosted Asisso stack to a newer image version"
---

This guide covers updating a Asisso stack you've already deployed with [Docker](/deployment/docker) or a [custom domain](/deployment/custom-domain). You run commands from the same directory that contains your `docker-compose.yaml` (this is the `asisso/` directory if you used `setup_remote.sh`).
...
Asisso publishes two images — `asisso-api` and `asisso-ui` — to both container registries:

- **GitHub Container Registry** — [github.com/orgs/asisso/packages](https://github.com/orgs/asisso/packages)
- **Docker Hub** — [hub.docker.com/u/asissoai](https://hub.docker.com/u/asissoai)
...
|-------|-----------|---------|-------------|
| GitHub release tag | `asisso-vX.Y.Z` | `asisso-v1.28.0` | What you see at [github.com/asisso/asisso/releases](https://github.com/asisso/asisso/releases) |
...
<Warning>
Always update **`asisso-api`** and **`asisso-ui`** to the **same tag**. The two images are built from the same commit and the UI expects API responses in a matching shape — mixing versions will break the app. `update_remote.sh` handles this for you automatically.
</Warning>
...
- Synchronizes the canonical remote keys in `.env` and validates the runtime config that `asisso-init` will render from it.
...
```bash
cd asisso
curl -o update_remote.sh https://raw.githubusercontent.com/asisso/asisso/main/scripts/update_remote.sh
bash update_remote.sh
```

You'll be prompted for the target version, defaulting to the most recent release. Accepted forms: bare semver (`1.28.0`), v-prefixed (`v1.28.0`), the full GitHub tag (`asisso-v1.28.0`), or `main` for bleeding edge — the script normalizes them. Non-interactive callers can set it via environment variable and skip the confirmation prompt:

```bash
TARGET_VERSION=1.28.0 ASISSO_UPDATE_YES=1 bash update_remote.sh
```
...
```bash
cd asisso
for f in docker-compose.yaml nginx.conf turnserver.conf .env remote_up.sh scripts/run_asisso_init.sh scripts/lib/setup_common.sh deploy/templates/nginx.remote.conf.template deploy/templates/turnserver.remote.conf.template; do
...
<Warning>
Rolling back across a database migration is not always safe — if the newer release ran a schema migration, downgrading may leave the DB in a state the older API doesn't understand. If in doubt, [open an issue](https://github.com/asisso/asisso/issues) before rolling back.
</Warning>
...
# Track latest main:
git pull
# Or pin to a specific release (git tag format is asisso-vX.Y.Z):
git checkout asisso-v1.28.0
