---
title: "Docker"
description: "Deploy Asisso using Docker for local development and remote server environments"
---

Asisso can be deployed using Docker in two main configurations. Choose the option that best fits your needs:

- **Option 1**: For local development and testing on your own machine
- **Option 2**: For remote server deployment with HTTPS (using IP address). If you also have a custom domain, you can first deploy Asisso stack on your server using steps in this document and then proceed to the [Custom Domain](deployment/custom-domain) section.

## Option 1: Local Docker Deployment

Watch the video tutorial below for a step-by-step walkthrough of setting up Asisso locally with Docker.

<iframe
  className="w-full aspect-video rounded-xl"
  src="https://www.tella.tv/video/asisso-local-setup-with-docker-5wsf/embed?b=1&title=1&a=1&loop=0&t=0&muted=0&wt=1"
  title="Asisso Local Setup with Docker"
  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
  allowFullScreen
></iframe>

For local development and testing, you can run Asisso directly on your machine using Docker with a single command.

### Quick Start

Run this single command to download and start Asisso:

```bash
curl -o docker-compose.yaml https://raw.githubusercontent.com/asisso/asisso/main/docker-compose.yaml && REGISTRY=ghcr.io/asisso ENABLE_TELEMETRY=true docker compose up --pull always
```
...
<CodeGroup>
```bash macOS/Linux
curl -o setup_local.sh https://raw.githubusercontent.com/asisso/asisso/main/scripts/setup_local.sh && chmod +x setup_local.sh && ./setup_local.sh
```
```powershell Windows
Invoke-WebRequest -OutFile setup_local.ps1 https://raw.githubusercontent.com/asisso/asisso/main/scripts/setup_local.ps1
.\setup_local.ps1
```
</CodeGroup>
...
It creates `docker-compose.yaml`, a `.env` file with TURN credentials, and the small helper bundle that `asisso-init` uses to render coturn config at startup. Start the stack with the `local-turn` profile so coturn comes up alongside the other services:
...
```bash
curl -o setup_remote.sh https://raw.githubusercontent.com/asisso/asisso/main/scripts/setup_remote.sh && chmod +x setup_remote.sh && ./setup_remote.sh
```
...
- Validate the runtime config that `asisso-init` will render from `.env`
...
After the setup script completes, start Asisso. The script prints the exact command to run at the end — it differs slightly between modes:
...
The setup script creates the following files in the `asisso/` directory:
...
| `scripts/run_asisso_init.sh` | One-shot init renderer/validator used by Docker Compose |
...
1. Clones the repo into `asisso/` (default: `asisso/asisso@main`; the script will ask for an `owner/name` and `branch` so a fork can point at its own).
2. If you run the script from inside an existing asisso checkout, offers to use that checkout instead of cloning.
