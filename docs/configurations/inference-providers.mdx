...
description: "Voice Agents need AI Models to work, like LLM (Large Language Model), TTS (Voice) and STT (Transcriber). You can use any of your faviourite providers with Asisso Platform to run your Voice Agent."
---

## How Model Configuration Works

Asisso uses a **two-level configuration system** for AI models:
...
The global configuration is the default model setup shared across all your agents. Asisso ships with its own models by default — when you sign up on [app.asisso.com](https://asisso.com/app) you get some Asisso model credits to start with.

To configure the global models, go to **Model Configurations** in your dashboard at [`app.asisso.com/model-configurations`](https://asisso.com/app/model-configurations).
...
Select a provider from the dropdown and configure the API key, model, and any provider-specific settings. For Asisso's own models, see [Service Keys](api-keys) for instructions on creating Service Keys.
...
Asisso supports Gemini 3.1 Live as a **Realtime** provider. The default model is `gemini-3.1-flash-live-preview`.
...
To use Gemini 3.1 Live with Asisso, you need a Google Gemini API key. Follow these steps:
...
<Note>
  The Gemini API key is different from a Google Cloud service account key. You specifically need a **Gemini API key** from Google AI Studio for use with Asisso.
</Note>

### Configuring Gemini 3.1 Live in Asisso

1. Go to **Model Configurations** in your Asisso dashboard at [`app.asisso.com/model-configurations`](https://asisso.com/app/model-configurations).
