...
| Field | Description |
| --- | --- |
| **Endpoint URL** | The URL Asisso will send the POST request to. |
...
Asisso sends a `POST` request with the following JSON payload:
...
```javascript
app.post("/asisso/pre-call", async (req, res) => {
