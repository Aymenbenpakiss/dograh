...
The id is shown on each row in **Telephony configurations** at [`app.asisso.com/telephony-configurations`](https://asisso.com/app/telephony-configurations). The configuration must belong to the same organization as the API Trigger; otherwise the request returns `404`.
