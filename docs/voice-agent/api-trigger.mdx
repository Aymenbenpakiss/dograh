...
The API Trigger node lets you initiate outbound calls to your voice agent programmatically. When you add an API Trigger node to your workflow, Asisso generates a unique endpoint URL that external systems can call to start a conversation.
...
The id is shown on each row in **Telephony configurations** (`https://app.asisso.com/telephony-configurations` for hosted or `http://localhost:3010/telephony-configurations` for local). The configuration must belong to the same organization as the API Trigger; otherwise the request returns `404`.
