...
---
<Note>
If the below steps do not work out for you, it would be great if you can open an issue on [Github](https://github.com/asisso/asisso/issues). 
</Note>
...
### Steps
1. Fork the Asisso repository by going to https://github.com/asisso/asisso
...
```
git clone https://github.com/<GITHUB_HANDLE>/asisso
cd asisso
```
3. Run the contributor bootstrap. It configures `origin` (your fork) and `upstream` (`asisso/asisso`), initializes the pipecat submodule, creates the Python venv, and copies the `.env` templates. Re-running it is safe — already-configured pieces are skipped.
<CodeGroup>
```bash macOS/Linux
...
9066b7244b2f   postgres:17   "docker-entrypoint.s…"   18 seconds ago   Up 18 seconds (healthy)   0.0.0.0:5432->5432/tcp, [::]:5432->5432/tcp   asisso-postgres-1
6c7cb8afdf18   redis:7       "docker-entrypoint.s…"   18 seconds ago   Up 18 seconds (healthy)   0.0.0.0:6379->6379/tcp, [::]:6379->6379/tcp   asisso-redis-1
a57e3e92b02c   minio/minio   "/usr/bin/docker-ent…"   18 seconds ago   Up 18 seconds (healthy)   127.0.0.1:9000-9001->9000-9001/tcp            asisso-minio-1
```
...
The bootstrap script configures two remotes: `origin` (your fork, where you push) and `upstream` (`asisso/asisso`, where new commits land). To pull in upstream changes:
...
```
origin    https://github.com/<YOUR_HANDLE>/asisso.git (fetch/push)
upstream  https://github.com/asisso/asisso.git    (fetch/push)
```

<Note>
Always push feature branches to **`origin`** (your fork), then open a pull request against `asisso/asisso:main`. Never push directly to `upstream`.
</Note>
